using ClientApplication_3.ZipServiceRef;
using System;
    ...
    class Program
    {
        ...
        static void ZipLookup(int _zipCode)
        {
            try
            {
                ...
                var endpoint = new EndpointAddress(new Uri(string.Format("http://localhost/ZipServiceRepXMLSerializer/ZipService.asmx", Environment.MachineName)));

                 ZipServiceClient service = new ZipServiceClient(binding, endpoint);

                var zipcodeInfo =  service.GetZipInfo(_zipCode.ToString());
